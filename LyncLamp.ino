...
void setup() {
  ...
  pinMode(PINK_BTN_PIN, INPUT_PULLUP);
...
int checkUserInput() {
  ...
  bool pinkBtnPressed = isPressed(PINK_BTN_PIN, &lastPinkBtnPressed);
...
void setStatusLEDs(int colorSelector) {
  for (int i = 0; i < strip.numPixels(); i++) {
    ...
    else if (colorSelector == COLOR_PINK) {
...
void showState(int state) {
  ...
  else if (state == STATE_PINK) {
    setStatusLEDs(COLOR_PINK);
  }
